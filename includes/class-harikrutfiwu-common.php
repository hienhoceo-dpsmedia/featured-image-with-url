...
class HARIKRUTFIWU_Common {
	...
	public function __construct() {
		...
		if ( ! is_admin() || ( defined( 'DOING_AJAX' ) && DOING_AJAX ) ) {
			...
			// Enhanced SEO Features by DPS.MEDIA
			$this->add_seo_enhancements();
		}
	...
	public function harikrutfiwu_facebook_for_woocommerce_support( $product_data, $product_id ) {
		...
		// Enhanced Product Image with SEO data by DPS.MEDIA
		...
		if ( isset( $product_image['img_url'] ) && ! empty( $product_image['img_url'] ) ) {
			$image_url = $product_image['img_url'];
			$product_data['image_url'] = $image_url;

			// Add image dimensions for Facebook optimization
			$image_size = $this->get_image_sizes( $image_url );
			if ( $image_size ) {
				$product_data['image_width'] = $image_size[0];
				$product_data['image_height'] = $image_size[1];
			}

			// Add alt text for accessibility and SEO
			$alt_text = get_post_meta( $product_id, '_harikrutfiwu_alt', true );
			if ( $alt_text ) {
				$product_data['image_alt_text'] = $alt_text;
			}

			// Support for Facebook image override
	...
	/**
	 * Add comprehensive SEO enhancements for external images.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	private function add_seo_enhancements() {
		// Enhanced Schema.org structured data for all post types
		add_action( 'wp_head', array( $this, 'add_enhanced_schema_markup' ), 5 );

		// Open Graph meta tags
		add_action( 'wp_head', array( $this, 'add_open_graph_tags' ), 10 );

		// Twitter Card meta tags
		add_action( 'wp_head', array( $this, 'add_twitter_card_tags' ), 15 );

		// Enhanced WooCommerce structured data
		add_filter( 'woocommerce_structured_data_product', array( $this, 'enhance_product_schema_data' ), 100, 2 );

		// Enhanced Facebook for WooCommerce support
		add_filter( 'facebook_for_woocommerce_integration_prepare_product', array( $this, 'enhance_facebook_product_data' ), 100, 2 );

		// Add JSON-LD schema for blog posts and pages
		add_action( 'wp_head', array( $this, 'add_article_schema' ), 8 );
	}

	/**
	 * Add enhanced Schema.org structured data for external images.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function add_enhanced_schema_markup() {
		...
		if ( ! $post_id || $this->harikrutfiwu_is_disallow_posttype( get_post_type( $post_id ) ) ) {
			return;
		}
		...
		if ( ! empty( $image_data['img_url'] ) ) {
			$image_schema = array(
				'@type' => 'ImageObject',
				'url' => esc_url( $image_data['img_url'] ),
				'contentUrl' => esc_url( $image_data['img_url'] ),
			);

			// Add image dimensions if available
			$image_size = $this->get_image_sizes( $image_data['img_url'] );
			if ( $image_size ) {
				$image_schema['width'] = $image_size[0];
				$image_schema['height'] = $image_size[1];
			}

			// Add alt text if available
			$alt_text = get_post_meta( $post_id, '_harikrutfiwu_alt', true );
			if ( $alt_text ) {
				$image_schema['name'] = esc_attr( $alt_text );
				$image_schema['description'] = esc_attr( $alt_text );
			}

			// Output as JSON-LD
			echo '<script type="application/ld+json">' . wp_json_encode( $image_schema, JSON_UNESCAPED_SLASHES ) . '</script>' . "\n";
		}
	}

	/**
	 * Add Open Graph meta tags for external images.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function add_open_graph_tags() {
		...
		if ( ! empty( $image_data['img_url'] ) ) {
			$image_url = esc_url( $image_data['img_url'] );
			...
			$image_size = $this->get_image_sizes( $image_data['img_url'] );
			if ( $image_size ) {
				echo '<meta property="og:image:width" content="' . esc_attr( $image_size[0] ) . '" />' . "\n";
				echo '<meta property="og:image:height" content="' . esc_attr( $image_size[1] ) . '" />' . "\n";
			}
			...
			// Get custom OG data if available
			$og_title = get_post_meta( $post_id, '_harikrutfiwu_og_title', true );
			$og_description = get_post_meta( $post_id, '_harikrutfiwu_og_description', true );
	...
	/**
	 * Add Twitter Card meta tags for external images.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function add_twitter_card_tags() {
		...
		if ( ! empty( $image_data['img_url'] ) ) {
			...
			// Get custom OG data for Twitter
			$og_title = get_post_meta( $post_id, '_harikrutfiwu_og_title', true );
			$og_description = get_post_meta( $post_id, '_harikrutfiwu_og_description', true );
	...
	/**
	 * Enhance WooCommerce product schema data with external images.
	 *
	 * @since 1.0.0
	 * @param array $markup Existing schema markup.
	 * @param WC_Product $product Product object.
	 * @return array Enhanced schema markup.
	 */
	public function enhance_product_schema_data( $markup, $product ) {
		if ( ! isset( $markup['image'] ) || empty( $markup['image'] ) ) {
			global $harikrutfiwu;
			$product_id = $product->get_id();
	...
	/**
	 * Enhance Facebook for WooCommerce product data with external images.
	 *
	 * @since 1.0.0
	 * @param array $product_data Product data for Facebook feed.
	 * @param int   $product_id   Product ID.
	 ...
	 */
	public function enhance_facebook_product_data( $product_data, $product_id ) {
		if ( empty( $product_data ) || empty( $product_id ) ) {
			return $product_data;
		}
		...
		if ( isset( $product_image['img_url'] ) && ! empty( $product_image['img_url'] ) ) {
			...
			// Add image dimensions for Facebook
			$image_size = $this->get_image_sizes( $image_url );
			if ( $image_size ) {
				$product_data['image_width'] = $image_size[0];
				$product_data['image_height'] = $image_size[1];
	...
	/**
	 * Add Article schema for blog posts and pages with external images.
	 *
	 * @since 1.0.0
	 * @return void
	 */
